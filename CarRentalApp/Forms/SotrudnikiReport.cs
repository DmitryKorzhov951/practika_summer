...
{
    /// <summary>
    /// Отчёт "Сотрудники" — карточки с цветовым выделением и рамками.
    /// </summary>
    public class SotrudnikiReport : Form
    {
        public SotrudnikiReport()
        {
            Text          = "Отчёт: Сотрудники";
            ...
            // Загрузка данных (все поля кроме PK)
            var dt = Db.Load(@"
                SELECT s.FIO          AS [ФИО],
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                foreach (DataColumn col in dt.Columns)
                {
                    card.Controls.Add(new Label
                    {
                        ...
                        AutoSize = false, Top = y, Left = 12, Width = 200, Height = 20
                    });
                    card.Controls.Add(new Label
                    {
                        Text = row[col]?.ToString() ?? "",
                        Font = new Font("Segoe UI", 9),
                        AutoSize = false, Top = y, Left = 220, Width = 580, Height = 20
                    });
                    y += 22;
                }

...
{
    /// <summary>
    /// При старте приложения проверяет наличие БД CarRentalDB,
    /// если её нет (или нет доступа) — создаёт заново под текущей Windows-учёткой.
    /// </summary>
    public static class DbInit
    {
        const string MasterConn =
            @"Server=(LocalDB)\MSSQLLocalDB;Database=master;Integrated Security=True;TrustServerCertificate=True;Connect Timeout=10";
        const string AppConn =
            @"Server=(LocalDB)\MSSQLLocalDB;Database=CarRentalDB;Integrated Security=True;TrustServerCertificate=True;Connect Timeout=10";
        ...
        public static void EnsureDatabase()
        {
            ...
            if (!dbAccessible)
            {
                ...
                {
                    throw new Exception(
                        "БД CarRentalDB зарегистрирована в LocalDB под другой Windows-учёткой " +
        ...
        /// <summary>Разбивает SQL по строкам "GO" (как делает SSMS).</summary>
        static IEnumerable<string> SplitByGo(string sql)
        {
            // Находим строки, содержащие только "GO" (с возможными пробелами)
            var parts = Regex.Split(sql, @"^\s*GO\s*$",
                                    RegexOptions.Multiline | RegexOptions.IgnoreCase);
            return parts;
        }

        // ===========================================================
        // SQL-скрипт инициализации (таблицы + данные + представления).
        // CREATE DATABASE здесь нет — БД уже создана выше.
        // ===========================================================
